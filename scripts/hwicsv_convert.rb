...
fixed = CSV.open(ARGV[1], 'wb', :headers => true, :return_headers => true)
...
headers.delete_at((headers.length - 1)) ## last header is null
...
csv.each_with_index do |row, i|
  ...
  ## delete the columns that we don't want/need
  discard_columns.each do |discard|
    row.delete(discard)
  end
  ...
  row.each_with_index do |column, i|
    ...
    if h == 'time'
      next
    elsif f.to_s.match(/Yes|No/)
      row[i] = f.to_s.length == 3 ? f = 1.0 : f = 0.0
    else
      row[i] = f.to_s.to_f
    end
  end
  ## the last header is null
  row.delete((row.length - 1))
  fixed << row
end
